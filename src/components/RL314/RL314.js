...
const RL314 = () => {
  ...
  const [statusValidasi, setStatusValidasi] = useState({value: 3,abel: "Belum divalidasi",});
    ...
    if (idrs !== "") {
      try {
        ...
        const results = await axiosJWT.get(
          "/apisirs/rltigatitikempatbelasadmin",
          customConfig
        );
        ...
        setNamaFile("RL314_" + idrs);
        setSpinner(false);
        setNamaRS(results.data.dataRS.RUMAH_SAKIT);
        if (kategoriUser === 3 && dataRLTigaTitikEmpatBelasDetails.length > 0) {
          setButtonStatus(false);
        } else if (
          kategoriUser === 3 &&
          dataRLTigaTitikEmpatBelasDetails.length === 0
        ) {
          setButtonStatus(true);
        }
        // setKabKota(results.)
    ...
    <div className="container" style={{ marginTop: "70px" }}>
      <div className="row">
        <div className="col-md-6">
          <div className="card">
            <div className="card-body">
              <h5 className="card-title h5">Validasi RL 3.14</h5>
        ...
        <div className="col-md-6">
          <div className="card">
            <div className="card-body">
              <h5 className="card-title h5">Filter RL 3.14</h5>
              <form onSubmit={Cari}>
              ...
              {dataRL.map((value, index) => {
                return (
                  <tr key={value.id}>
                    ...
                    <td>
                      <label htmlFor="">{value.rujukan_diterima_dari_puskesmas}</label>
                    </td>
                    <td>
                      <label htmlFor="">{value.rujukan_diterima_dari_fasilitas_kesehatan_lain}</label>
                    </td>
                    <td>
                      <label htmlFor="">{value.rujukan_diterima_dari_rs_lain}</label>
                    </td>
                    <td>
                      <label htmlFor="">{value.rujukan_dikembalikan_ke_puskesmas}</label>
                    </td>
                    <td>
                      <label htmlFor="">{value.rujukan_dikembalikan_ke_fasilitas_kesehatan_lain}</label>
                    </td>
                    <td>
                      <label htmlFor="">{value.rujukan_dikembalikan_ke_rs_asal}</label>
                    </td>
                    <td>
                      <label htmlFor="">{value.dirujukan_pasien_rujukan}</label>
                    </td>
                    <td>
                      <label htmlFor="">{value.dirujuk_pasien_datang_sendiri}</label>
                    </td>
                    <td>
                      <label htmlFor="">{value.dirujuk_diterima_kembali}</label>
                    </td>
                  </tr>
                );
              })}
